...
window.SistemaAuth = {
    ...
    _validarELogin: function(uid, usuario, senha, callback) {
        const authEmail = usuario.authEmail || usuario.email;
        if (!authEmail) {
            callback(false, "Credenciais inválidas");
            return;
        }

        // Autentica via Firebase Auth usando o e-mail interno gerado no cadastro.
        this.auth.signInWithEmailAndPassword(authEmail, senha)
            .then(() => {
